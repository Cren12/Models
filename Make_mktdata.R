...
Make_mktdata <- function(
  ohlc, # An OHLC object
  change # Minimum price movement in percent 
)
{
  min.points <- FindMins(hl = ohlc[, c('High', 'Low')],
                         change = change)
  ...
  X <- foreach(n = 5:250) %dopar%
  {
    ...
    value <- cbind.data.frame(volat, roc, sma)
    colnames(value) <- paste0(c('volat', 'roc', 'sma'), '.', n)
    return(value)
  }
  ...
  value <- cbind.data.frame(min.points, X)
